...
export function PackagesPage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="bg-[#14356B] py-10 md:py-14 text-white">
        <div className="max-w-[1400px] mx-auto px-4 md:px-6">
          <div className="text-[#C9A227] text-[12px] font-bold uppercase tracking-widest mb-2">All Packages</div>
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-6">
          ...
          <div className="flex gap-3 mb-4 md:mb-8">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full pl-9 pr-4 py-3 md:py-2.5 border border-[#E5E7EB] rounded-[10px] text-[13px] text-[#111827] bg-white outline-none focus:border-[#14356B] focus:ring-2 focus:ring-[#14356B]/10 min-h-[48px]" />
            </div>
            ...
            <button
              ...
              className={cn(
                ...
                filtersOpen ? "bg-[#14356B] text-white border-[#14356B]" : "bg-white text-[#374151] border-[#E5E7EB]"
              )}>
              <Filter size={15} /> Filters
              {type !== "All" && <span className="w-4 h-4 bg-[#C9A227] text-[#14356B] rounded-full text-[9px] font-black flex items-center justify-center">1</span>}
            </button>
          </div>
              ...
              {TYPES.map(t => (
                <button key={t} onClick={() => { setType(t); setFiltersOpen(false); }}
                  className={cn("px-4 py-2.5 rounded-full text-[12px] font-bold transition-all cursor-pointer min-h-[44px]",
                    type === t ? "bg-[#14356B] text-white" : "bg-white text-[#6B7280] border border-[#E5E7EB] hover:border-[#14356B]/30"
                  )}>
                  {t}
                </button>
              ))}
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-[#9CA3AF]">
              ...
              <button onClick={() => { setType("All"); setSearch(""); }} className="mt-3 text-[13px] text-[#14356B] font-bold hover:underline cursor-pointer">
                Clear filters
              </button>
            </div>
          ) : (
              ...
              {filtered.map(pkg => (
                <Link key={pkg.id} to={`/packages/${pkg.id}`}
                  ...
                  <div className="relative h-52 overflow-hidden">
                    ...
                    {pkg.badge && (
                      <div className="absolute top-3 left-3 bg-[#C9A227] text-[#14356B] text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wide">
                        {pkg.badge}
                      </div>
                    )}
                    ...
                    <div className="absolute bottom-3 left-3 bg-[#14356B]/80 backdrop-blur-sm rounded-full px-2.5 py-1 text-[10px] text-white font-bold">
                      {pkg.type}
                    </div>
                  </div>
                  <div className="p-5">
                    <h3 className="text-[14px] font-bold text-[#111827] group-hover:text-[#14356B] transition-colors mb-2 leading-snug">{pkg.title}</h3>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-[#F3F4F6]">
                      <div>
                        ...
                        <div className="text-[19px] font-black text-[#14356B]">{fmtPrice(pkg.price)}</div>
                      </div>
  ...
  if (!pkg) {
    return (
      <div className="py-32 text-center">
        ...
        <Link to="/packages" className="mt-4 inline-block text-[#14356B] font-bold hover:underline">← Back to Packages</Link>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <section className="relative h-[240px] sm:h-[320px] md:h-[400px] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#14356B]/90 via-[#14356B]/40 to-transparent" />
                ...
                {(["overview", "itinerary", "inclusions"] as const).map(t => (
                  <button key={t} onClick={() => setTab(t)}
                    className={cn("px-5 py-2 rounded-[8px] text-[12px] font-bold capitalize transition-all cursor-pointer",
                      tab === t ? "bg-[#14356B] text-white shadow" : "text-[#9CA3AF] hover:text-[#374151]"
                    )}>
                    {t}
                  </button>
                ))}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {[
                        ...
                        <div key={d.label} className="flex flex-col gap-1 bg-[#F7F8FA] rounded-[10px] p-3">
                          <d.icon size={15} className="text-[#14356B]" />
                  ...
                  <div className="flex flex-col gap-4 relative">
                    ...
                    {pkg.itinerary.map((day, i) => (
                      <div key={i} className="flex gap-5 relative">
                        <div className="w-8 h-8 rounded-full bg-[#14356B] text-white text-[11px] font-black flex items-center justify-center flex-shrink-0 z-10 border-4 border-white shadow">
                          {i + 1}
                        </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl border border-[#E5E7EB] p-6 sticky top-24 shadow-lg">
                <div className="flex items-baseline gap-1 mb-1">
                  <div className="text-[28px] font-black text-[#14356B]">{fmtPrice(pkg.price)}</div>
                ...
                <form className="flex flex-col gap-3" onSubmit={e => e.preventDefault()}>
                  <div>
                    ...
                    <input placeholder="Your name" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                      className="w-full px-3 py-2.5 border border-[#E5E7EB] rounded-[8px] text-[12px] outline-none focus:border-[#14356B] focus:ring-2 focus:ring-[#14356B]/10" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select value={form.pax} onChange={e => setForm(f => ({ ...f, pax: e.target.value }))}
                      className="w-full px-3 py-2.5 border border-[#E5E7EB] rounded-[8px] text-[12px] outline-none focus:border-[#14356B] bg-white cursor-pointer">
                      {["1","2","3","4","5","6+"].map(n => <option key={n}>{n}</option>)}
                    </select>
                  </div>
                  <button type="submit" className="w-full py-3 bg-[#14356B] hover:bg-[#0F2A55] text-white font-black rounded-[10px] text-[13px] transition-colors cursor-pointer mt-1">
                    Book This Package
                  </button>
