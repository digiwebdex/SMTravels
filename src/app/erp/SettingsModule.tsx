...
function Panel({ title, icon: Icon, iconColor="#14356B", children, className }: {
  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={()=>setOn(v=>!v)}
        className={cn("relative w-10 h-5 rounded-full transition-colors", on?"bg-[#14356B]":"bg-slate-300")}>
  ...
  return (
    <div className="flex justify-end mt-5">
      <button onClick={()=>{ setOk(true); setTimeout(()=>setOk(false),2200); }}
        className="flex items-center gap-2 px-5 py-2 text-sm bg-[#14356B] text-white rounded-lg hover:bg-[#0f2a56]">
    ...
    <div className="space-y-5">
      ...
      <Panel title="Branding" icon={Star}>
        <Field label="Primary Color">
          <div className="flex gap-2"><input type="color" defaultValue="#14356B" className="w-10 h-9 rounded-lg border border-slate-200 p-0.5 cursor-pointer"/><Inp dv="#14356B"/></div>
        </Field>
        <Field label="Company Logo">
          <div className="flex items-center gap-3">
            <div className="w-16 h-10 rounded-lg bg-[#14356B] flex items-center justify-center text-white text-xs font-bold">BDH</div>
    ...
    <div className="space-y-5">
      ...
      <Panel title="OCR Provider" icon={ScanText} iconColor="#7C3AED">
        ...
        <Field label="Confidence Threshold" hint="Min % to auto-accept field">
          <div className="flex items-center gap-3">
            <input type="range" min={50} max={99} defaultValue={85} className="flex-1 accent-[#14356B]"/>
      ...
      <Panel title="Monthly Usage" icon={Star}>
        ...
        <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden mb-1.5">
          <div className="h-full bg-[#14356B] rounded-full" style={{ width:"62.4%" }}/>
        </div>
        <p className="text-xs text-slate-400">Resets Aug 1, 2024 · <button className="text-[#14356B] hover:underline">Upgrade Plan</button></p>
      </Panel>
      <SaveBtn/>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Backup Settings" subtitle="Automated database and file backup"
        action={<button className="flex items-center gap-2 px-4 py-2 text-sm bg-[#14356B] text-white rounded-lg"><Download size={14}/> Backup Now</button>}/>
...
const ROLES_DATA = [
  ...
  { id:2, name:"Branch Manager", users:3, color:"#14356B", desc:"Full branch access, read-only other branches."     },
  { id:3, name:"Sales Executive",users:8, color:"#0E7C66", desc:"Manage bookings, clients, invoices."               },
  { id:4, name:"Visa Officer",   users:4, color:"#C9A227", desc:"Visa applications, OCR, status updates."           },
  ...
  return (
    <div>
      <PageHeader title="Role Management" subtitle="Define roles and their access scope"
        action={<button className="flex items-center gap-2 px-4 py-2 text-sm bg-[#14356B] text-white rounded-lg"><Plus size={14}/> New Role</button>}/>
      <div className="grid grid-cols-3 gap-5">
        <div className="space-y-2">
          {ROLES_DATA.map(r=>(
            <button key={r.id} onClick={()=>setSel(r.id)}
              className={cn("w-full text-left p-4 rounded-xl border transition-all",
                sel===r.id?"border-[#14356B] bg-[#14356B]/5":"bg-white border-slate-200 hover:bg-slate-50")}>
...
const P_CELL: Record<Perm,string> = {
  full: "bg-[#14356B] text-white",
  view: "bg-[#14356B]/15 text-[#14356B]",
  ...
  return (
    <div>
      <PageHeader title="Branch Management" subtitle="Head office and regional branches"
        action={<button className="flex items-center gap-2 px-4 py-2 text-sm bg-[#14356B] text-white rounded-lg"><Plus size={14}/> Add Branch</button>}/>
      <div className="space-y-3">
        {BRANCH_DATA.map((b,i)=>(
          <div key={i} className={cn("bg-white rounded-xl border p-5",b.hq?"border-[#14356B]/30 bg-[#14356B]/3":"border-slate-200")}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-bold",b.hq?"bg-[#14356B]":"bg-slate-400")}>{b.city.slice(0,2)}</div>
                <div>
                  <div className="flex items-center gap-2">
                    <p className="font-semibold text-slate-800">{b.name}</p>
                    {b.hq && <span className="text-xs px-2 py-0.5 bg-[#14356B] text-white rounded-full font-medium">HQ</span>}
                    <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium",
...
const U_ROLE_COLOR: Record<string,string> = {
  "Super Admin":"#EF4444","Branch Manager":"#14356B","Sales Executive":"#0E7C66",
  "Visa Officer":"#C9A227","Accountant":"#2563EB","Support Staff":"#64748B",
};
  ...
  return (
    <div>
      <PageHeader title="Users Management" subtitle="Staff accounts, roles, and access control"
        action={<button className="flex items-center gap-2 px-4 py-2 text-sm bg-[#14356B] text-white rounded-lg"><Plus size={14}/> Invite User</button>}/>
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table className="w-full">
...
const PLANS = [
  ...
  { id:"pro",      name:"Pro",        price:4900, users:15, branches:3, color:"#14356B", current:true,
    features:["Everything in Starter","WhatsApp integration","OCR 200/mo","Advanced reports","Priority support"] },
  { id:"business", name:"Business",   price:9900, users:50, branches:10,color:"#C9A227", current:false,
      ...
      <div className="grid grid-cols-4 gap-4 mb-5">
        {PLANS.map(p=>(
          <div key={p.id} className={cn("rounded-xl border p-5 relative",
            p.current?"border-[#14356B] bg-[#14356B]/3":"bg-white border-slate-200")}>
            {p.current && <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-[#14356B] text-white text-xs font-bold rounded-full whitespace-nowrap">Current Plan</div>}
            ...
            <button className={cn("w-full py-2 text-sm font-medium rounded-lg transition-colors",
              p.current?"bg-[#14356B] text-white cursor-default":"border border-slate-200 text-slate-600 hover:bg-slate-50")}>
          ...
          {[{label:"Users",used:18,max:15,over:true},{label:"Branches",used:3,max:3,over:false},{label:"Storage",used:12,max:50,sfx:" GB",over:false}].map(r=>(
            <div key={r.label} className="bg-slate-50 rounded-xl p-3">
              ...
              <div className="h-2 bg-slate-200 rounded-full overflow-hidden">
                <div className={cn("h-full rounded-full",r.over?"bg-red-500":"bg-[#14356B]")}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[{l:"CPU Usage",v:"24%",b:24,c:"bg-[#14356B]"},{l:"Memory",v:"58%",b:58,c:"bg-[#0E7C66]"},{l:"Disk I/O",v:"12%",b:12,c:"bg-[#C9A227]"},{l:"Network",v:"8 MB/s",b:35,c:"bg-[#2563EB]"}].map(s=>(
