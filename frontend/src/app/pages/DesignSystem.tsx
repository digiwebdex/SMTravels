      ...
      <header className="sticky top-0 z-30 bg-white border-b border-slate-200">
        <div className="max-w-[1300px] mx-auto px-6 py-3.5 flex items-center gap-6">
          ...
          <nav className="hidden md:flex items-center gap-1 overflow-x-auto no-scrollbar">
            {NAV_SECTIONS.map(s => (
              <a key={s.id} href={`#${s.id}`}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-500 hover:text-[#1B75BC] hover:bg-[#1B75BC]/5 transition-all whitespace-nowrap">
                <s.icon size={12} />
                {s.label}
              </a>
            ))}
          ...
          <div className="ml-auto flex items-center gap-2">
              ...
              {(["en", "bn"] as const).map(l => (
                <button key={l} onClick={() => setLang(l)}
                  className={cn("px-3 py-1.5 text-xs font-bold rounded-[10px] transition-all",
                    lang === l ? "bg-white text-[#1B75BC] shadow-sm" : "text-slate-500")}>
                  {l === "en" ? "English" : "বাংলা"}
                </button>
              ))}
            </div>
            ...
            <Link to="/" className="flex items-center gap-1.5 text-xs font-semibold text-white bg-[#1B75BC] px-3 py-2 rounded-xl hover:bg-[#14588F]">
              <Home size={12} /> Home
            </Link>
          </div>
        </div>
          ...
          <div className="space-y-10">
            <SubSection title="Brand Colors">
              <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-8 gap-3">
                <ColorSwatch name="Navy"       hex="#1B75BC" token="--brand-navy" />
                <ColorSwatch name="Navy Dark"  hex="#14588F" token="hover state" />
                <ColorSwatch name="Gold"       hex="#F15A24" token="--brand-gold" />
            ...
            <SubSection title="Semantic Colors">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { name: "Warning",  hex: "#F15A24", bg: "#FFF9E6", label: "Pending, Follow-up" },
              ...
              <div className="flex flex-wrap gap-4 items-end">
                {[2, 4, 6, 8, 10, 12, 16, 20, 24].map(n => (
                  <div key={n} className="flex flex-col items-center gap-1">
                    <div className="bg-[#1B75BC]" style={{ width: n * 4, height: n * 4, borderRadius: 2 }} />
              ...
              <div className="mt-4 flex flex-wrap gap-4 items-center">
                {[4, 8, 12, 16, 20, 9999].map(r => (
                  <div key={r} className="flex flex-col items-center gap-1">
                    <div className="w-12 h-8 bg-[#1B75BC]/20 border-2 border-[#1B75BC]"
            ...
            <SubSection title="Avatars & Utility">
              <Demo label="Avatars">
                <div className="flex items-end gap-4">
                  ...
                  <Avatar name="Shah Jalal" color="#F15A24" size="lg" />
              ...
              <Demo label="Tags & Tooltip">
                <div className="flex flex-wrap gap-2">
                  <Tag label="Hajj 2026" />
                  <Tag label="VIP" color="#F15A24" />
                  <Tag label="Dhaka HQ" color="#1B75BC" onRemove={() => {}} />
          ...
          <div className="space-y-10">
            ...
            <SubSection title="Sidebar Nav Item">
              <Demo label="Active / Hover / Default states">
                <div className="bg-[#1B75BC] rounded-xl w-52 p-2 space-y-0.5">
                  {[
                    ...
                    {[
                      ...
                      return (
                        <div key={i} className="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl min-w-[52px] relative cursor-pointer"
                          style={{ minHeight: 44 }}>
                          <Icon size={22} className={item.active ? "text-[#1B75BC]" : "text-slate-400"} />
                          <span className={cn("text-[10px] font-semibold leading-none",
                            item.active ? "text-[#1B75BC]" : "text-slate-400")}>{item.label}</span>
                          {item.badge && (
                          ...
                          {item.active && <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 bg-[#1B75BC] rounded-full" />}
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={u.rule} className="flex items-center gap-3 px-3 py-2.5 bg-slate-50 rounded-xl">
                    <code className="text-xs font-mono text-[#1B75BC] bg-[#1B75BC]/5 px-2 py-1 rounded font-bold">{u.rule}</code>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    <div key={item.label} className="bg-white rounded-xl border border-slate-200 p-4">
                      ...
                      <p className="text-sm font-bold text-[#1B75BC]">{item.bn}</p>
                    </div>
                  ))}
                </div>
              </Demo>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(["short", "medium", "long"] as const).map(style => (
                    <div key={style} className="bg-white rounded-xl border border-slate-200 p-4">
                      ...
                      <p className="text-sm font-semibold text-[#1B75BC]">{formatDate("2026-01-15", style, "bn")}</p>
                    </div>
                  ))}
                </div>
              </Demo>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={en} className="flex items-center justify-between bg-white rounded-xl border border-slate-200 px-3 py-2.5">
                      ...
                      <span className="text-sm font-semibold text-[#1B75BC]">{bn}</span>
                    </div>
                  ))}
                </div>
              </Demo>
      ...
      <footer className="border-t border-slate-200 bg-white mt-16">
        <div className="max-w-[1300px] mx-auto px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/erp" className="text-xs text-slate-400 hover:text-[#1B75BC] transition-colors">ERP</Link>
            <Link to="/login" className="text-xs text-slate-400 hover:text-[#1B75BC] transition-colors">Login</Link>
          </div>
        </div>
      </footer>
    </div>
