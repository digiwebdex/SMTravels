...
const SERVICES = [
  { id: "hajj", icon: Star, label: "Hajj Package", color: "#C9A227", desc: "Govt-approved pilgrimage packages" },
  { id: "umrah", icon: MapPin, label: "Umrah Package", color: "#14356B", desc: "Year-round Umrah services" },
          ...
          <div className="flex flex-col items-center">
            <div className={cn("w-9 h-9 rounded-full flex items-center justify-center text-[12px] font-black transition-all",
              i < current ? "bg-[#0E7C66] text-white"
                : i === current ? "bg-[#14356B] text-white shadow-lg"
                  : "bg-[#E5E7EB] text-[#9CA3AF]"
            )}>
              {i < current ? <CheckCircle size={16} /> : i + 1}
            </div>
            <div className={cn("text-[10px] font-bold mt-1 hidden sm:block",
              i === current ? "text-[#14356B]" : i < current ? "text-[#0E7C66]" : "text-[#9CA3AF]"
            )}>
              {step}
            </div>
          </div>
...
export function BookingPage() {
      ...
      <div className="min-h-[70vh] flex items-center justify-center bg-[#F7F8FA]">
        <div className="max-w-md mx-auto px-6 text-center">
          ...
          <div className="flex gap-3 justify-center">
            <Link to="/" className="px-5 py-2.5 bg-[#14356B] text-white font-bold rounded-[10px] text-sm hover:bg-[#0F2A55] transition-colors">
              Back to Home
            </Link>
  ...
  return (
    <>
      <section className="bg-[#14356B] py-12 text-white text-center">
        <div className="max-w-[700px] mx-auto px-6">
          <div className="text-[#C9A227] text-[12px] font-bold uppercase tracking-widest mb-2">Online Booking</div>
      ...
      <section className="py-12 bg-[#F7F8FA] min-h-[70vh]">
        <div className="max-w-[700px] mx-auto px-4 md:px-6">
                  ...
                  {SERVICES.map(s => (
                    <button key={s.id} onClick={() => setService(s.id)}
                      className={cn(
                        ...
                        service === s.id ? "border-[#14356B] bg-[#14356B]/5 shadow" : "border-[#E5E7EB] hover:border-[#14356B]/30"
                      )}>
                      ...
                      {service === s.id && <CheckCircle size={14} className="text-[#14356B] mt-2" />}
                    </button>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <textarea rows={3} placeholder="Wheelchair access, dietary restrictions, specific package preferences..."
                      ...
                      className="px-3 py-2.5 border border-[#E5E7EB] rounded-[10px] text-[13px] outline-none focus:border-[#14356B] resize-none transition-all" />
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="date" value={traveler.dob} onChange={e => setTr("dob")(e.target.value)}
                      className="px-3 py-2.5 border border-[#E5E7EB] rounded-[10px] text-[13px] outline-none focus:border-[#14356B] transition-all" />
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-between mt-7 pt-5 border-t border-[#F3F4F6]">
              {step > 0 ? (
                <button onClick={() => setStep(s => s - 1)}
                  className="flex items-center gap-2 px-5 py-2.5 border-2 border-[#E5E7EB] text-[#374151] font-bold rounded-[10px] text-[13px] hover:border-[#14356B]/30 transition-all cursor-pointer">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    step === 0 && !service
                      ...
                      : "bg-[#14356B] hover:bg-[#0F2A55] text-white"
                  )}>
          ...
          <div className="mt-5 text-center text-[12px] text-[#9CA3AF]">
            Need help? <a href="https://wa.me/8801712345678" target="_blank" rel="noopener noreferrer" className="text-[#25D366] font-bold hover:underline">Chat on WhatsApp</a> or call <a href="tel:+88029553421" className="text-[#14356B] font-bold hover:underline">+880 2 9553421</a>
          </div>
        </div>
      </section>
    </>
