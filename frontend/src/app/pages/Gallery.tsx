...
export function GalleryPage() {
  ...
  const filtered = active === "All" ? GALLERY_IMAGES : GALLERY_IMAGES.filter(g => g.category === active);
    ...
    <>
      ...
      <section className="py-12 bg-[#F7F8FA]">
        <div className="max-w-[1400px] mx-auto px-6">
          ...
          {/* Masonry-style grid — 2 col mobile, 3 tablet, 4 desktop */}
          <div className="columns-2 md:columns-3 lg:columns-4 gap-3 md:gap-4 space-y-3 md:space-y-4">
            {filtered.map((g, i) => (
              <div key={g.id} className="break-inside-avoid relative group cursor-pointer rounded-[12px] overflow-hidden shadow-sm border border-[#E5E7EB]"
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center px-4"
          ...
          <div className="max-w-4xl max-h-[85vh] w-full" onClick={e => e.stopPropagation()}>
            <img src={img(filtered[lightbox].image, 1200, 800)} alt={filtered[lightbox].title}
