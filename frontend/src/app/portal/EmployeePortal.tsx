...
import {
  ...
  Play, Square, Plus, Phone, Mail, Building2, Briefcase,
} from "lucide-react";
...
import {
  useHrMe, useMyLeaveRequests, useCreateMyLeaveRequest, useLeaveTypes,
  useMyAttendance, useClockIn, useClockOut, downloadEmployeeDocument,
} from "../hooks/hr";

type PortalView = "profile" | "documents" | "leave" | "attendance" | "notifications";
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-lg)] divide-y divide-[var(--color-border-subtle)]">
          {docs.map((d) => (
            <div key={d.id} className="flex items-center justify-between px-4 py-3">
              ...
              {me && (
                <button type="button" onClick={() => downloadEmployeeDocument(me.id, d)} aria-label={`Download ${d.title}`}
