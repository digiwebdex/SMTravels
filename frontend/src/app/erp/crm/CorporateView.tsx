  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={openNew} className="flex items-center gap-1.5 h-9 px-4 bg-[#0E6BB8] text-white rounded-[8px] text-[12px] font-bold hover:bg-[#0B5794] transition-colors cursor-pointer shadow-lg shadow-[#0E6BB8]/20"><Plus size={14} /> New Corporate Client</button>
      </div>

      <StatCards items={[
        { label: "Corporate Clients", value: String(total), icon: Building2, color: "#0E6BB8", bg: "#EEF2FF" },
        ...
        { label: "This Page", value: String(rows.length), icon: User, color: "#E8471F", bg: "#FFF9E6" },
      ]} />
      ...
      <Card className="mb-4 p-4">
        <div className="flex items-center gap-3">
          <div className="flex-1 relative">
            ...
            <input placeholder="Search by company, contact, phone…" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-3 h-9 bg-[#F7F8FA] border border-[#E5E7EB] rounded-[8px] text-[13px] outline-none focus:border-[#0E6BB8] focus:ring-2 focus:ring-[#0E6BB8]/10 placeholder:text-[#D1D5DB]" />
          </div>
