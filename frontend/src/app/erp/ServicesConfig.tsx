  ...
  return (
    <div className="p-5 md:p-7">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: "Total Services",   value: svcs.length.toString(),           color: "#14356B", bg: "#EEF2FF" },
          { label: "Active",           value: activeCount.toString(),            color: "#0E7C66", bg: "#ECFDF5" },
          { label: "Total Bookings",   value: svcs.reduce((s, v) => s + v.stats.bookings, 0).toLocaleString(), color: "#C9A227", bg: "#FFF9E6" },
              ...
              return (
                <Card key={svc.id} className={cn("p-5 transition-all hover:shadow-md", !svc.active && "opacity-70")}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          {svc.featured && (
                            <span className="text-[9px] font-bold text-[#C9A227] bg-[#FFF9E6] px-1.5 py-0.5 rounded-full">FEATURED</span>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <button onClick={() => onConfigure(svc.id)}
                    className="w-full h-9 flex items-center justify-center gap-1.5 border border-[#E5E7EB] rounded-[8px] text-[12px] font-semibold text-[#374151] hover:border-[#14356B]/40 hover:text-[#14356B] hover:bg-[#EEF2FF]/50 transition-all cursor-pointer">
    ...
    <div className="p-5 md:p-7">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div className="flex items-center gap-1.5 text-[13px]">
          <button onClick={onBack} className="text-[#14356B] font-semibold hover:underline cursor-pointer">Services</button>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-5 pb-4 border-b border-[#F3F4F6]">
            ...
            <button onClick={addField}
              className="flex items-center gap-1.5 h-8 px-3 bg-[#EEF2FF] text-[#14356B] font-bold rounded-[8px] text-[11px] hover:bg-[#14356B] hover:text-white transition-colors cursor-pointer">
              <Plus size={12} /> Add Field
            </button>
          </div>
          <div className="space-y-2">
            {fields.map((field, i) => (
              <div key={field.id} className="flex items-start gap-3 p-3.5 bg-[#F7F8FA] rounded-[10px] border border-[#E5E7EB] group hover:border-[#14356B]/20 transition-colors">
                {/* Order */}
                ...
                <div className="w-6 h-6 rounded-full bg-[#14356B] text-white text-[9px] font-black flex items-center justify-center flex-shrink-0 mt-0.5">
                  {field.order}
                </div>
          ...
          <div className="mt-4 p-3 bg-[#FFF9E6] border border-[#C9A227]/30 rounded-[8px] flex items-center gap-2 text-[11px] text-[#78590F]">
                  ...
                  <tr key={rule.id} className="border-t border-[#F3F4F6] group">
                    <td className="py-2.5 px-3">
                      <input value={rule.label} onChange={e => updatePricing(rule.id, { label: e.target.value })}
                        className="w-36 px-2 py-1.5 border border-[#E5E7EB] rounded-[7px] text-[12px] font-semibold text-[#111827] focus:border-[#14356B] outline-none" />
                    </td>
                    <td className="py-2.5 px-3">
                      <select value={rule.type} onChange={e => updatePricing(rule.id, { type: e.target.value })}
                        className="w-28 px-2 py-1.5 border border-[#E5E7EB] rounded-[7px] text-[12px] text-[#374151] focus:border-[#14356B] outline-none cursor-pointer bg-white">
                    ...
                    <td className="py-2.5 px-3">
                      <input type="number" value={rule.amount} onChange={e => updatePricing(rule.id, { amount: +e.target.value })}
                        className="w-28 px-2 py-1.5 border border-[#E5E7EB] rounded-[7px] text-[12px] font-mono text-[#111827] focus:border-[#14356B] outline-none" />
                    </td>
                    <td className="py-2.5 px-3">
                      <select value={rule.currency} onChange={e => updatePricing(rule.id, { currency: e.target.value })}
                        className="w-20 px-2 py-1.5 border border-[#E5E7EB] rounded-[7px] text-[12px] text-[#374151] focus:border-[#14356B] outline-none cursor-pointer bg-white">
                        {["BDT","USD","SAR"].map(c => <option key={c}>{c}</option>)}
                      </select>
                    </td>
                    <td className="py-2.5 px-3">
                      <input value={rule.note} onChange={e => updatePricing(rule.id, { note: e.target.value })}
                        className="w-48 px-2 py-1.5 border border-[#E5E7EB] rounded-[7px] text-[11px] text-[#6B7280] focus:border-[#14356B] outline-none" placeholder="Note…" />
                    </td>
                    <td className="py-2.5 px-3">
              ...
              <div className="space-y-1.5 mt-1">
                {["Bank Transfer","Mobile Banking (bKash/Nagad)","Cash at Branch","Online Payment Gateway"].map(m => (
                  <label key={m} className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" defaultChecked className="w-3.5 h-3.5 accent-[#14356B]" />
              ...
              <div key={doc.id} className="p-4 bg-[#F7F8FA] rounded-[10px] border border-[#E5E7EB] group">
                <div className="flex items-start gap-3">
                  <div className="w-7 h-7 rounded-[8px] flex items-center justify-center flex-shrink-0 mt-1" style={{ backgroundColor: doc.required ? "#EEF2FF" : "#F3F4F6" }}>
                    <FileText size={13} style={{ color: doc.required ? "#14356B" : "#9CA3AF" }} />
                  </div>
                  <div className="flex-1 grid grid-cols-2 md:grid-cols-5 gap-3">
                    <div className="md:col-span-2">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={step.step} className="flex items-center gap-3 p-3 bg-[#F7F8FA] rounded-[8px]">
                  <div className="w-6 h-6 rounded-full bg-[#14356B] text-white flex items-center justify-center text-[10px] font-black flex-shrink-0">
                    {step.step}
                  </div>
                  <div className="flex-1">
              ...
              <button className="w-full h-8 border-2 border-dashed border-[#D1D5DB] rounded-[8px] text-[11px] font-semibold text-[#9CA3AF] hover:border-[#14356B]/50 hover:text-[#14356B] transition-colors cursor-pointer flex items-center justify-center gap-1">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {notif.channels.map(ch => (
                      <span key={ch} className="text-[9px] font-bold text-[#14356B] bg-[#EEF2FF] px-2 py-0.5 rounded-full">{ch}</span>
                    ))}
                  </div>
                </div>
